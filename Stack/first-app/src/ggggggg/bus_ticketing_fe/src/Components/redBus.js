...
function RedBus() {
  ...
  const availableSeats=(id) =>{
    console.log("hiiiiiiii");

   
    axios
    .post('http://localhost:5110/api/Booking/BookedSeats', {
      busId:id,
      date:selectedDate
    })
    .then((response) => {
      console.log(response.data.availableSeats);
      
     return response.data.availableSeats;
    })
    .catch((err) => {
      console.error(err);
      setSearchError('Error searching available seats. Please try again.');
    });
  }
